...
## The rule that makes this skill honest

**Not every scenario becomes a unit test, and pretending otherwise is worse than writing nothing.** A fabricated test that passes against a fake while the real behaviour is untested is a false green — it actively removes the pressure to test the thing properly.

So classify every scenario before writing anything:
...
Report the classification per scenario and say plainly which scenarios end up with no automated coverage. **That list is the useful output** — it tells the reader what the tests do *not* prove.
...
## Step 3 — Read a sibling before writing
...
## Step 5 — Verify the tests actually fail
